...
/*리턴값: 0 = 정상, -1 = error*/
/*---------------------*/
int command(unsigned char memory[][MEM_ROW], List *history_list, List **op_table, List **sym_table, char **token, int token_number, char *history_temp)	//history_temp : to handle history. if input is "hi" or "history",
{																																		//input the command into the history list before print it.
	...
	if(token_number == 1) {					//help, dir, quit, history, dump, reset, opcodelist
		if(!strcmp(token[0], "h") || !strcmp(token[0], "help")) {
			...
			free_hashTable(op_table, 20);
			free_hashTable(sym_table, 40);
			exit(0);	
		} else if (!strcmp(token[0], "hi") || !strcmp(token[0], "history")) {
			list_insert(history_list, history_temp, 0, not_format);
		...
		if(!strcmp(token[0], "opcode")) {
			op_code = find_opcode(op_table, token[1], NULL);
			if(op_code == -1) {
			...
			status = type_file(token[1]);
		} else if(!strcmp(token[0], "assemble")) {
			table_initialize(sym_table, 40);
			status = overall_assemble(token[1], op_table, sym_table);	
		} else {
			status = handle_error(wrong_input);
		}
